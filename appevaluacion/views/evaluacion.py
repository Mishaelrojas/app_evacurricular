...
def listar_evaluaciones(request):
    try:
        evaluador = request.user.evaluador  # Obtener el usuario
    except ObjectDoesNotExist:
        # Si el usuario no tiene un evaluador asociado, mostrar un mensaje de error
    ...
    if queryset:
        datos = datos.filter(Q(codigo_evaluacion__icontains=queryset)).distinct().order_by('idEvaluacion').values()  
    
    datos = datos.order_by('idEvaluacion')
    paginator = Paginator(datos, 10)  
    page_number = request.GET.get('page')
    page_obj = paginator.get_page(page_number)    
    return render(request, 'evaluacion/listar.html', {'page_obj': page_obj, 'estado_actual': estado_evaluacion})
...
def reporte_postulacion_pdf(request, pk):
    ...
    for detalle in detalles_evaluacion:
        resultado = {
            ...
            'puntaje_proyInvestig': detalle.puntaje_proyInvestig,
            'puntaje_asesoriaProy': detalle.puntaje_asesoriaProy,
    ...
    #response['Content-Disposition'] = 'attachment; filename="reporte_postulacion.pdf"'
